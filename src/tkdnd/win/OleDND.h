...
public:
  TkDND_DropTarget(Tcl_Interp *_interp, Tk_Window _tkwin) : interp(_interp), tkwin(_tkwin), m_lRefCount(1)
  {
    szTempStr[0] = 0;
  }; /* TkDND_DropTarget */

  ~TkDND_DropTarget(void){}; /* ~TkDND_DropTarget */
                         ...
                         POINTL pt, DWORD *pdwEffect)
  {
    ...
    IEnumFORMATETC *pEF;
    FORMATETC fetc;
    Tcl_Obj *typelist = Tcl_NewListObj(0, NULL),
    ...
    *pdwEffect = DROPEFFECT_NONE;
    ...
    case ActionDefault:
      *pdwEffect = DROPEFFECT_COPY;
      break;
    case refuse_drop:
      *pdwEffect = DROPEFFECT_NONE; /* Refuse drop. */
    }
    return S_OK;
  }; /* DragEnter */
  ...
  {
    Tcl_Obj *objv[5], *pressedkeys = Tcl_NewListObj(0, NULL);
    int i, status, index;
    enum dropactions
    {
      ActionCopy,
    ...
    for (type_index = 0; type_index < typeObjc; ++type_index)
    {
      ...
      if (status == TCL_OK)
      {
        ...
        case TYPE_CF_UNICODETEXT:
          data = GetData_CF_UNICODETEXT(pDataObject);
          break;
        case TYPE_CF_TEXT:
          data = GetData_CF_TEXT(pDataObject);
          break;
        case TYPE_CF_HDROP:
          data = GetData_CF_HDROP(pDataObject);
          break;
...
#define BLOCK_SIZE 1024
  ...
  Tcl_Obj *GetData_FileGroupDescriptor(IDataObject *pDataObject)
  {
    ...
    if (pDataObject->QueryGetData(&descriptor_fmt) != S_OK)
      return NULL;
    if (pDataObject->QueryGetData(&contents_fmt) != S_OK)
      return NULL;
  ...
  Tcl_Obj *GetData_FileGroupDescriptorW(IDataObject *pDataObject)
  {
         ...
         file_index < file_group_descriptor->cItems; file_index++)
    {
      ...
      if (pDataObject->GetData(&contents_fmt, &content_storage) == S_OK)
      {
        ...
        Tcl_GetUnicode(item);
        Tcl_AppendUnicodeToObj(item, (Tcl_UniChar *)file_descriptor.cFileName, -1);
        GlobalLock(content_storage.pstm);

...
inline bool posHasIQP(Position *pos, colorT c)
{
    ...
    return (pos->FyleCount(p, C_FYLE) == 0 && pos->FyleCount(p, E_FYLE) == 0 && pos->FyleCount(p, D_FYLE) > 0);
}
...
inline bool posHasKPawnStorm(Position *pos, colorT c)
{
    ...
    if (c == WHITE)
    {
        return (bd[G2] != WP && bd[H2] != WP && bd[G3] != WP && bd[H3] != WP && (pos->FyleCount(WP, G_FYLE) > 0 || pos->FyleCount(WP, H_FYLE) > 0));
    }
    return (bd[G7] != BP && bd[H7] != BP && bd[G6] != BP && bd[H6] != BP && (pos->FyleCount(BP, G_FYLE) > 0 || pos->FyleCount(BP, H_FYLE) > 0));
}
...
void OpLine::Init(void)
{
    ...
    for (uint t = 0; t < NUM_POSTHEMES; t++)
    {
        Theme[t] = 0;
    }
...
void OpLine::Init(Game *g, IndexEntry *ie, gameNumberT gameNum,
                  uint maxExtraMoves, uint maxThemeMoveNumber)
{
    ...
    if (maxLineMoves > OPLINE_MOVES)
    {
        maxLineMoves = OPLINE_MOVES;
    }
...
void OpLine::SetPositionalThemes(Position *pos)
{
    ...
    if (posHasOpenFyle(pos, C_FYLE) || posHasOpenFyle(pos, D_FYLE) || posHasOpenFyle(pos, E_FYLE))
    {
...
uint OpLine::CommonLength(OpLine *line)
{
    ...
    {
        if (!strEqual(Move[length], line->Move[length]))
        {
            break;
        }
        if (strEqual(Move[length], ""))
        {
            break;
        }
    }
...
void OpLine::PrintMove(DString *dstr, const char *move, uint format)
{
    char tempTrans[5000];
...
void OpLine::PrintNote(DString *dstr, uint movenum, uint start, uint format)
{
    bool wtm = true;
...
void OpLine::PrintSummary(DString *dstr, uint format, bool fullDate, bool nmoves)
{
    ...
    if (WhiteElo > 0)
    {
        dstr->Append(preElo, WhiteElo, postElo);
    }
    ...
    if (BlackElo > 0)
    {
        dstr->Append(preElo, BlackElo, postElo);
    }
    ...
    if (fullDate)
    {
        ...
        char *q = (char *)strFirstChar(dateStr + 4, '?');
        if (q != NULL)
        {
            q--;
            *q = 0;
...
void OpTable::Init(uint base, const char *type, Game *g, PBook *ebook)
{
    Base = base;
    ...
    while (!g->AtStart())
    {
        ...
        g->GetCurrentPos()->MakeSANString(sm, StartLine[StartLength],
                                          SAN_CHECKTEST);
        StartLength++;
        if (StartLength >= OPTABLE_MAX_STARTLINE)
        {
            break;
        }
    }
    ...
    MaxThemeMoveNumber = 0;
    NumNotes = 0;
    Line[OPTABLE_MAX_LINES - 1] = {0};
    MoveOrder[OPTABLE_MAX_LINES - 1] = {0, 0, 0};
    Row[OPTABLE_MAX_TABLE_LINES - 1] = {0};
    NLines[OPTABLE_MAX_TABLE_LINES - 1] = {0};
    RowScore[OPTABLE_MAX_TABLE_LINES - 1] = {0};
}
...
uint OpTable::PercentScore(void)
{
    ...
    percent = percent * 500;
    uint total = Results[RESULT_White] + Results[RESULT_Draw] + Results[RESULT_Black];
    if (total > 0)
    {
...
uint OpTable::TheoryPercent(void)
{
    ...
    percent = percent * 500;
    uint total = TheoryResults[RESULT_White] + TheoryResults[RESULT_Draw] + TheoryResults[RESULT_Black];
    if (total > 0)
    {
        percent = percent / total;
    }
...
bool OpTable::Add(OpLine *line)
{
    ...
    {
        if (line->Theme[theme] >= POSTHEME_THRESHOLD)
        {
            ThemeCount[theme]++;
        }
    }
    ...
    if (WTM && strEqual(line->Move[0], ExcludeMove))
    {
        return OK;
    }
    if (!WTM && strEqual(line->Move[1], ExcludeMove))
    {
        return OK;
    }
    ...
    if (NumLines < OPTABLE_MAX_LINES)
    {
        Line[NumLines] = line;
        NumLines++;
        if (NumTableLines < MaxTableLines)
        {
            NumTableLines++;
        }
        return OK;
...
void OpTable::MergeRow(uint rownum)
{
...
void OpTable::MakeRows(void)
{
    ...
    if (TargetRows < OPTABLE_MIN_ROWS)
    {
        TargetRows = OPTABLE_MIN_ROWS;
    }
    if (TargetRows > OPTABLE_MAX_ROWS)
    {
...
void OpTable::SortTableLines(OpLine **lines, uint nlines, uint depth)
{
    uint i, j, nUnique = 0;
    if (nlines < 2)
    {
    ...
    for (i = 0; i < nUnique - 1; i++)
    {
        for (j = i + 1; j < nUnique; j++)
        {
            ...
            if (strEqual(moves[i].move, ""))
            {
                result = -1;
            }
            if (strEqual(moves[j].move, ""))
            {
                result = 1;
            }
            // Swap if necessary:
...
void OpTable::PrintTable(DString *dstr, const char *title, const char *comment)
{
...
void OpTable::PrintLaTeX(DString *dstr, const char *title, const char *comment)
{
    uint i;
    MakeRows();
    NumNotes = 0;
    ...
    for (uint row = 0; row < NumRows; row++)
    {
        ...
        if (row > 0)
        {
            nSameMoves = Row[row]->CommonLength(Row[row - 1]);
        }
...
void OpTable::PrintHTML(DString *dstr, const char *title, const char *comment)
{
    uint i;
    MakeRows();
    NumNotes = 0;
...
void OpTable::PrintText(DString *dstr, const char *title, const char *comment,
                        bool ctext)
{
    uint i;
    MakeRows();
    ...
    for (uint row = 0; row < NumRows; row++)
    {
        ...
        for (uint j = 0; j < 2 * OPTABLE_COLUMNS; j++)
        {
            ...
            if (HasNotes(Row[row], j))
            {
                ...
                if (ctext)
                {
                    dtemp->Append("</N", NumNotes, "></go></red>");
                }
            }
...
bool OpTable::HasNotes(OpLine *line, uint movenum)
{
...
void OpTable::PrintNotes(DString *dstr, uint format)
{
    if (NumNotes == 0)
    {
        return;
...
void OpTable::BestGames(DString *dstr, uint count, const char *rtype)
{
    enum
    {
        BEST_White,
...
void OpTable::TopPlayers(DString *dstr, colorT c, uint count)
{
    uint i;
    ...
    for (i = 0; i < NumLines; i++)
    {
        ...
        if (year != 0)
        {
            ...
            if (year < pf[id].minYear)
            {
                pf[id].minYear = year;
            }
            if (year > pf[id].maxYear)
            {
                pf[id].maxYear = year;
            }
        }
        if (Line[i]->NoteNumber != 0)
        {
            for (uint n = 0; n <= PLAYERFREQ_MAXNOTES; n++)
            {
                if (pf[id].noteNumber[n] == Line[i]->NoteNumber)
                {
                    break;
                }
    ...
    for (uint n = 1; n <= count; n++)
    {
        ...
        if (found)
        {
            ...
            if (pf[index].noteNumber[0] != 0)
            {
                ...
                for (uint pn = 0; pn < PLAYERFREQ_MAXNOTES; pn++)
                {
                    if (pf[index].noteNumber[pn] == 0)
                    {
                        break;
                    }
                    if (pn > 0)
                    ...
                    if (Format == OPTABLE_CText)
                    {
                        dstr->Append("<go n", pf[index].noteNumber[pn]);
                        dstr->Append(">", pf[index].noteNumber[pn], "</go>");
                    }
                    else
                    {
                        dstr->Append(pf[index].noteNumber[pn]);
                    }
                }
...
void OpTable::TopEcoCodes(DString *dstr, uint count)
{
    uint ecoCount[50];
    uint ecoScore[50];
...
uint OpTable::AvgElo(colorT color, uint *count, uint *oppScore, uint *oppPerf)
{
    uint n = 0;
    uint sum = 0;
    uint score = 0;
...
uint OpTable::AddMoveOrder(Game *g)
{
    ...
    if (index < 0)
    {
        if (NumMoveOrders == OPTABLE_MAX_LINES)
        {
            return 0;
        }
...
void OpTable::PopularMoveOrders(DString *dstr, uint count)
{
...
void OpTable::ThemeReport(DString *dstr, uint argc, const char **argv)
{
    const char *endLine = "\n";
    const char *percentStr = "%";
    const char *startTable = "";
    ...
    if (Format == OPTABLE_HTML)
    {
        ...
        endRow = "</td></tr>\n";
        nextCell = "</td><td>";
        nextCellRight = "</td><td align=\"right\">";
        //startTable = "<pre>\n"; endTable = "</pre>\n";
    }
    if (Format == OPTABLE_CText)
    {
...
void OpTable::AddEndMaterial(matSigT msig, bool inFilter)
{
...
void OpTable::EndMaterialReport(DString *dstr, const char *repGames,
                                const char *allGames)
{
    const char *startTable = "";
    const char *endTable = "";
    ...
    if (Format == OPTABLE_LaTeX)
    {
        ...
        dstr->Append(x, x, x, r);
        dstr->Append(x, q, x, qr);
        dstr->Append(endRow, startRow);
        dstr->Append(p, bn, r, bn);
        dstr->Append(q, bn, qr, bn);
    ...
    else
    {
        ...
        if (length[OPTABLE_All] > len)
        {
            len = length[OPTABLE_All];
        }
        len++;
    ...
    for (uint t = OPTABLE_Line; t <= OPTABLE_All; t++)
    {
        ...
        for (i = 0; i < NUM_EGTHEMES; i++)
        {
            ...
            if (sum > 0)
            {
                pc = ((100 * EndgameCount[t][i]) + (sum / 2)) / sum;
            }
...
OpTable::SelectGames(char type, uint number)
{
    ...
    for (uint i = 0; i < NumLines; i++)
    {
        ...
        else if (type == 't')
        {
            ...
            if (number < NUM_POSTHEMES && line->Theme[number] >= POSTHEME_THRESHOLD)
            {
                selected = true;
            }
        }

...
void SpellChecker::Init(void)
{
    ...
    for (uint h = 0; h < SPELL_HASH_SIZE; h++)
    {
        HashNames[h] = NULL;
    }
    Prefixes = NULL;
    Suffixes = NULL;
    Infixes = NULL;
    NameType = NAME_INVALID;
}

void SpellChecker::Clear(void)
{
    Destroy();
    Init();
}

void SpellChecker::Destroy(void)
{
    delete[] ExcludeChars;
    delete StrAlloc;
    StrAlloc = NULL;
    for (uint i = 0; i < 256; i++)
    {
        ...
        while (node != NULL)
        {
            ...
            if (node->renderName != NULL)
            {
                delete[] node->renderName;
            }
...
void SpellChecker::SetExcludeChars(const char *str)
{
    delete[] ExcludeChars;
    ExcludeChars = strDuplicate(str);
}

void SpellChecker::SetRenderName(spellCheckNodeT *node, const char *name)
{
...
uint SpellChecker::Corrections(const char *name, const char **corrections,
                               uint maxCorrections)
{
...
void SpellChecker::AddBioData(spellCheckNodeT *node, const char *str)
{
...
//
void SpellChecker::AddEloData(spellCheckNodeT *node, const char *str)
{
    ASSERT(node != NULL && str != NULL);
    if (node->eloData == NULL)
    {
...
void SpellChecker::SetElo(spellCheckNodeT *node,
                          uint year, uint yIndex, eloT elo)
{
    // Monitor array bounds

    if (year < ELO_YEAR_FIRST || year > ELO_YEAR_LAST)
    {
        return;
    }
    if (yIndex >= ELO_RATINGS_PER_YEAR)
    {
        return;
    }
...
//
eloT SpellChecker::GetElo(const char *name, dateT date, bool exact)
{
...
SpellChecker::GetTitle(const char *comment)
{
    ...
    while (*titlePtr != NULL)
    {
        if (strIsPrefix(*titlePtr, comment))
        {
            return *titlePtr;
        }
        titlePtr++;
...
eloT SpellChecker::GetPeakRating(const char *comment)
{
    if (comment == NULL)
    {
        return 0;
...
dateT SpellChecker::GetBirthdate(const char *comment)
{
    if (comment == NULL)
    {
        return ZERO_DATE;
...
//    field, convert it to a date, and return it.
dateT SpellChecker::GetDeathdate(const char *comment)
{
    if (comment == NULL)
    {
        return ZERO_DATE;
